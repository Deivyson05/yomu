![Capa](https://raw.githubusercontent.com/Deivyson05/yomu/main/proj-yomu.png)

# yomu
Repositório dedicado ao Projeto Integrador 3 da Universidade Católica de Pernambuco. Contém código-fonte, documentação e materiais desenvolvidos ao longo do semestre, aplicando conhecimentos práticos e teóricos adquiridos no curso.

**Integrantes:**
- Ingrid Beatriz Silva,
- Deivyson Ricardo Silva dos Santos,
- Iwerson Guilherme da Silva Souza,
- Vinicius de Almeida Silva,
- João Vitor Ferreira da Silva,
- Artur Francisco Damascena,
- André Luís Gomes da Silva Filho,
- José Heitor Felix Guimarães

Obs(O projeo esta dividio em branchs)

## Sobre

Yomu é um aplicativo de gamificação da leitura criado para ajudar jovens a manterem o foco e desenvolverem o hábito de ler livros físicos.
O app utiliza sistemas de progresso, desafios e recompensas para tornar a leitura mais envolvente, divertida e constante.

### 📚 Objetivo

O Yomu transforma a leitura em uma experiência motivadora, incentivando o usuário a:

- Ler todos os dias
- Registrar seu progresso de forma simples
- Completar metas e desafios
- Desbloquear conquistas e ganhar pontos
- Criar uma rotina saudável de leitura

### 🎯 Público-alvo

Jovens que têm dificuldade em manter concentração ou consistência na leitura, e que desejam tornar o hábito mais leve e recompensador.

### ⭐ Principais recursos

- Registro de leitura de livros físicos
- Sistema de níveis, XP e rankings
- Monitoramento de tempo de leitura
- Interface leve e orientada à motivação
